...
class BondsYields:
    ...
    @st.cache_data(show_spinner=False)
    def get_bonds_futures(_self):

        data_24 = pd.read_csv(_self.config.vars.par_yield_curves_2024)
        data_25 = pd.read_csv(_self.config.vars.par_yield_curves_2025)

        data = pd.concat([data_24, data_25], ignore_index=True)
        data.set_index("Date", inplace=True)

        data.drop(columns="1.5 Month", inplace=True)
    ...
    def generate_graphs(self):
        ...
        if selected_indicator == "Rendimentos das Treasuries":
            ...
            with st.spinner("Carregando os dados..."):
                ...
                yields_data = self.get_bonds_futures()

                min_value = round(min([
                    float(yields_data.values[i][j]) 
                    for i in range(len(yields_data.values)) 
                    for j in range(len(yields_data.values[0]))]) - 0.3, 3)
